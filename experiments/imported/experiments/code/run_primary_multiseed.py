...
PRETRAINED = os.environ.get("RESNET18_MODEL", str(DEFAULT_PRETRAINED))
SCALES = (1, 2, 4, 8)
SEEDS = (101, 202, 303)
GAMMA = 0.7
...
def aider_data(split_path: Path | None = None):
    split_path = split_path or (
        ROOT / "experiments/derived/aider_rcg/aider_splits.json"
    )
    splits = json.loads(
...
def hurricane_data(split_path: Path | None = None):
...
def run_dataset(
    ...
    aider_splits: Path | None = None,
    hurricane_splits: Path | None = None,
) -> dict:
    data = (
        aider_data(aider_splits)
        if name == "aider"
        else hurricane_data(hurricane_splits)
    )
    (
        train_items,
        val_items,
...
def main() -> None:
    ...
    parser.add_argument(
        "--aider-splits",
        type=Path,
        default=ROOT / "experiments/derived/aider_rcg/aider_splits.json",
    )
    parser.add_argument("--hurricane-splits", type=Path)
    parser.add_argument(
        "--output",
        type=Path,
    ...
    for name in args.datasets:
        ...
        summary[name] = run_dataset(
            ...
            aider_splits=args.aider_splits,
            hurricane_splits=args.hurricane_splits,
        )
